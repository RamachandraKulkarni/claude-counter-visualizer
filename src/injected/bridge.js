(() => {
	...
	/**
	 * Post message with strict origin validation.
	 * [SECURITY] Only emit to claude.ai origin.
	 */
	function post(type, payload) {
		window.postMessage({ cc: CC_MARKER, type, payload }, ALLOWED_ORIGIN);
	}

	/**
	 * Post response with strict origin validation.
	 * [SECURITY] Only emit to claude.ai origin.
	 */
	...
	async function handleConversationResponse({ orgId, conversationId }, response) {
		try {
			...
			// [ERROR-HANDLING] Log parse failures (best effort)
			console.warn('[CC] Failed to parse conversation response:', e?.message);
		}
	}
			...
			while (true) {
				...
				for (const line of lines) {
					...
					try {
						...
						// SSE line parse failure is expected and safe to ignore
					}
				}
			}
		} catch (e) {
			// [ERROR-HANDLING] Log but don't break claude.ai
			console.warn('[CC] Event stream handling error:', e?.message);
		}
	}
